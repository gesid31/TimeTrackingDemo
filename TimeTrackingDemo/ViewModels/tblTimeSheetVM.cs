...
namespace TimeTrackingDemo.ViewModels
{
    public class tblTimeSheetVM
    {
        ...
        public List<tblTimeSheet> GetTimeSheet(string user)
        {
            ...
            tblTimeSheet tt = new tblTimeSheet();
            tt.tblTimeSheetsList = new List<tblTimeSheet>();
            if (dataSet != null && dataSet.Tables.Count > 0 && dataSet.Tables[0].Rows.Count > 0)
            {

                foreach (DataRow dr in dataSet.Tables[0].Rows)
                {
                    tt.tblTimeSheetsList.Add(new tblTimeSheet(dr["UserName"].ToString(), Convert.ToDateTime(dr["Date"]), Convert.ToInt32(dr["ID"]), Convert.ToDateTime(dr["TimeSheetIN"]), Convert.ToDateTime(dr["TimeSheetOUT"] == DBNull.Value ? null : (DateTime?)Convert.ToDateTime(dr["TimeSheetOUT"])), dr["Status"].ToString(), Convert.ToInt32(dr["TotalHours"] == DBNull.Value ? 0 : Convert.ToInt32(dr["TotalHours"]))));
                }

            }
            return tt.tblTimeSheetsList;
        }
    }
}